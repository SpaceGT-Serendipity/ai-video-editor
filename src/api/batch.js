...
export function getOptimumBatch(support) {
  return axios({
    method: "get",
    url:
      import.meta.env.VITE_APP_BATCH_SERVER +
      "/batch/getOptimumBatch/" +
      support,
  });
}
...
export function subtitleJob(batch, options) {
  return axios({
    method: "post",
    url: batch.path + "/subtitle-job",
    headers: {
      "Content-Type": "application/json",
    },
    responseType: "blob",
...
export const steps = [
  ...
  {
    name: "unit-video-add-subtitles-step",
    label: "视频添加字幕",
  },
  {
